...
-- ./ghci.bat C:\Users\piotr_justyna\Documents\github\programming-in-haskell\part1_chapter7_exercise8
-- option 2 (stack):
-- stack ghci
-- option 3 (ghci):
-- ghci
...
main = do
    ...
    putStrLn "transmitWithParityBitUsingAFaultyChannel \"Haskell\""
    putStrLn . show $ transmitWithParityBitUsingAFaultyChannel "Haskell"
...
decodeWithParityBit bits =
    if (length bits) `mod` 9 /= 0
    then error "The total number of bits is expected to be divisible by 9."
    else map (chr . bit2Int . checkParityBit . make9) (chop9 bits)
...
transmitWithParityBitUsingAFaultyChannel :: String -> String
transmitWithParityBitUsingAFaultyChannel = decodeWithParityBit . faultyChannel . encodewithParityBit
