...
-- ./ghci.bat C:\Users\piotr_justyna\Documents\github\programming-in-haskell\part2_chapter11_exercise2
-- option 2 (stack):
-- stack ghci
-- option 3 (ghci):
-- ghci
...
nextPlayer O = X
nextPlayer X = O
nextPlayer Blank = error "Argument does not apply in the context of \"next\""
        ...
        do  i <- getNaturalNumber (prompt player)
            case (move grid i player) of
                [] -> do
                    ...
                    run' grid player
                [newGrid] -> run newGrid (nextPlayer player)

prompt :: Player -> String
prompt player = "Player " ++ (show player) ++ ", enter your move:"

gameTree :: Grid -> Player -> Tree Grid
gameTree grid player = Node grid [gameTree newGrid (nextPlayer player) | newGrid <- moves grid player]
...
randomMove grid player = do
    index <- randomRIO (0, (length moves) - 1)
    return (moves !! index)
    where
        prunedTree = prune depth  (gameTree grid player)
        Node (_, bestPlayer) newTree = minimax prunedTree
        moves = [newGrid | Node (newGrid, newPlayer) newTreeSubtree <- newTree, newPlayer == bestPlayer]
    ...
    | player == O = do
        ...
        case move grid index player of
            [] -> do
                putStrLn "ERROR: Invalid move."
                play' grid player
            [newGrid] -> play newGrid (nextPlayer player)
    | player == X = do
        newMove <- randomMove grid player
        (play $! newMove) (nextPlayer player)