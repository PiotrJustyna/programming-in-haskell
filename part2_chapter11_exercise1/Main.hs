...
-- ./ghci.bat C:\Users\piotr_justyna\Documents\github\programming-in-haskell\part2_chapter11_exercise1
-- option 2 (stack):
-- stack ghci
-- option 3 (ghci):
-- ghci
...
main = do
    ...
    putStrLn $ "Game tree nodes: " ++ (show (numberOfGameTreeNodes (Node x grid)))
    putStrLn $ "Game tree depth: " ++ (show (length grid))
    where
        Node x grid = gameTree empty O
        maximumDepth = length grid

numberOfGameTreeNodes :: Tree Grid -> Int
numberOfGameTreeNodes (Node x []) = 1
numberOfGameTreeNodes (Node x subTrees) = 1 + (sum [numberOfGameTreeNodes singleSubTree | singleSubTree <- subTrees])
