...
-- ./ghci.bat C:\Users\piotr_justyna\Documents\github\programming-in-haskell\part2_chapter11_exercise4
-- option 2 (stack):
-- stack ghci
-- option 3 (ghci):
-- ghci
...
tictactoe :: IO ()
tictactoe = do
    putStrLn "Would you like to play first (F) or second (S)? To exit, press (E)."
    line <- getLine
    case line of
        "f" -> play empty O
        "s" -> play empty X
        "e" -> putStrLn "Bye!"
        other -> do
            putStrLn $ "Unexpected input, please try again."
            tictactoe
